...
export default class TextBackgroundColor implements InlineTool {
  ...
  private customSurround(color: string) {
    const termWrapper = this.api.selection.findParentTag(this.tag, TextBackgroundColor.CSS);
  ...
  public wrap(color?: string) {
    ...
    span.classList.add(TextBackgroundColor.CSS);
  ...
  public checkState(): boolean {
    const termTag = this.api.selection.findParentTag(this.tag, TextBackgroundColor.CSS);
  ...
  private updateToolboxIcon() {
    if (this.button) {
      ...
      if (this.currentColor === '#ffffff' || this.currentColor === 'rgb(255, 255, 255)') {
